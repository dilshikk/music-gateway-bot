...
async def cmd_admin(message: Message, pool: UserbotPool, queue: QueueManager) -> None:
    ...
    await message.answer(
        ...
        f"🤖 Userbot: <b>{pool_stats['idle']}</b> свободных / "
        f"<b>{pool_stats['total']}</b> всего\n"
        f"⏳ Очередь: <b>{queue_stats.get('queue_size', 0)}</b> запросов\n"
        f"🔴 FloodWait: <b>{pool_stats['flood']}</b>\n"
        f"❌ Ошибок: <b>{pool_stats['error']}</b>",
...
async def admin_userbots(callback: CallbackQuery, pool: UserbotPool) -> None:
    ...
    await callback.message.edit_text(
        f"<b>🤖 Userbot'ы</b> ({len(entries)} шт.)\n\n"
...
async def admin_userbot_detail(
    ...
    text = (
        f"<b>Userbot #{m.id}</b>\n\n"
        f"📱 Телефон: <code>{m.phone}</code>\n"
        f"📊 Статус: <b>{m.status.value}</b>\n"
        f"⚖️ Вес: {m.weight}\n"
...
async def admin_ub_add_phone(message: Message, state: FSMContext) -> None:
    ...
    await message.answer("🔑 Введите <code>api_id</code> (число):")
...
async def admin_ub_add_api_id(message: Message, state: FSMContext) -> None:
    ...
    await message.answer("🔑 Введите <code>api_hash</code>:")
...
async def admin_ub_add_api_hash(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(
        "📋 Введите <code>session_string</code>:\n\n"
        "<i>Получить можно через: python -c \"from pyrogram import Client; "
        "Client('s').start()\"</i>"
    )
